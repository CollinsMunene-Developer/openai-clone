...
import { AuthFormState, ResetPasswordFormState, UpdatePasswordFormState } from '@/types/auth'
  ...
  try {
    ...
    if (statusData?.exists) {
      return {
        ...
        verificationPending: !statusData.is_verified,
        email: email,
        success: false,
    ...
    // Proceed with signup for new user
 ...
 */
export async function auth(mode: 'login' | 'signup', prevState: AuthFormState, formData: FormData) {
  if (mode === 'login') {
 ...
 */
export async function resendVerification(prevState: AuthFormState, formData: FormData): Promise<AuthFormState> {
  const email = formData.get('email') as string
...
export async function updatePassword(prevState: UpdatePasswordFormState, formData: FormData): Promise<UpdatePasswordFormState> {
  ...
  if (password !== confirmPassword) {
    return {
      errors: {
        confirmPassword: AUTH_ALERTS.FORM.VALIDATION.PASSWORDS_DONT_MATCH.message
      }
    }
  }
